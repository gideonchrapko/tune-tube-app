...
export default function HomePage({ session }: { session: any }) {
  return (
    <main className="bg-[#f0f4f7] h-screen flex flex-col justify-center items-center relative perspective-[1000px] overflow-hidden">
      ...
      <Link
        href="/login"
        className="absolute top-5 right-5 px-[30px] py-[10px] bg-black text-white text-sm font-bold cursor-pointer uppercase tracking-[2px] border-none rounded-full transition-transform duration-300 ease-in-out hover:transform hover:scale-105"
      >
        Sign In
      </Link>
